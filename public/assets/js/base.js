//-----------------------------------------------------------------------
// Version:        2.1
// Template name:  laksamedical - Wallet & Banking HTML Mobile Template
// Item URL :      https://themeforest.net/item/laksamedical-wallet-banking-html-mobile-template/25738217
// Author:         Bragher
...
goBackButton.forEach(function (el) {
    el.addEventListener("click", function () {
        if (laksamedical.Animation.goBack) {
            goBackAnimation();
        }
        else {
            window.history.go(-1);
...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("laksamedicalAddtoHome");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        }
        else {
            localStorage.setItem("laksamedicalAddtoHome", 1)
...
//-----------------------------------------------------------------------
// Dark Mode
var checkDarkModeStatus = localStorage.getItem("laksamedicalDarkmode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode");

// Check if enable as default
if (laksamedical.Dark_Mode.default) {
    pageBody.classList.add("dark-mode");
}

// Local Dark Mode
if (laksamedical.Dark_Mode.local_mode.enable) {
    var nightStart = laksamedical.Dark_Mode.local_mode.start_time;
    var nightEnd = laksamedical.Dark_Mode.local_mode.end_time;
...
if (laksamedical.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function (el) {
    el.addEventListener("click", function () {
        var darkmodeCheck = localStorage.getItem("laksamedicalDarkmode");
        var bodyCheck = pageBody.classList.contains('dark-mode');
        if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
            pageBody.classList.remove("dark-mode");
            localStorage.setItem("laksamedicalDarkmode", "0");
            switchDarkModeCheck(false);
        }
        else {
            pageBody.classList.add("dark-mode")
            switchDarkModeCheck(true);
            localStorage.setItem("laksamedicalDarkmode", "1");
        }
    })
})
//-----------------------------------------------------------------------
...
function testMode() {
    ...
    console.clear();
    console.log("%claksamedical", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #6236FF; padding: 10px 120px; margin-bottom: 16px;")
    ...
    testModeMsg(laksamedical.PWA.enable, "PWA")
    testModeMsg(laksamedical.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(laksamedical.Dark_Mode.local_mode.enable, "Local dark mode (between " + laksamedical.Dark_Mode.local_mode.start_time + ":00 and " + laksamedical.Dark_Mode.local_mode.end_time + ":00)")
    testModeMsg(laksamedical.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(laksamedical.RTL.enable, "RTL")
    testModeMsg(laksamedical.Test.enable, "Test mode")
    testModeMsg(laksamedical.Test.alert, "Test mode alert")
    ...
    testModeMsg(laksamedical.Animation.goBack, "Go Back")
}
function themeTesting() {
    var word = laksamedical.Test.word;
    var value = "";
        ...
        if (value == word || value === word) {
            ...
            if (laksamedical.Test.alert) {
                ...
                var alert =
                    ...
                    +
                    laksamedical.Test.alertMessage
                    +
                    "</strong></div></div></div>"
