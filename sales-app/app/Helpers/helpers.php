...
function format_indo($date) {
    ...
    $bulan = $bulan[(int)substr($tanggal, 5, 2)];
...
if (!function_exists('get_salam')) {
  function get_salam($time_info = null) {
      // Gunakan waktu sekarang jika $time_info tidak diberikan
      $time_info = $time_info ?? date('H:i');
      
      // Tentukan salam dan informasi kerja berdasarkan waktu
      if ($time_info > '06:30' && $time_info < '10:59') {
          return ['salam' => 'Pagi', 'time_info_kerja' => 'Masuk'];
      } elseif ($time_info >= '11:00' && $time_info < '14:59') {
          return ['salam' => 'Siang', 'time_info_kerja' => 'Pulang'];
      } elseif ($time_info >= '15:00' && $time_info < '18:59') {
          return ['salam' => 'Sore', 'time_info_kerja' => null];
      } else {
          return ['salam' => 'Malam', 'time_info_kerja' => null];
      }
  }
}
...
function sensor_kata($filter){
    $filter = preg_replace("/jelek/", "**", $filter);
    $filter = preg_replace("/tai/", "**", $filter);
    $filter = preg_replace("/penipu/", "**", $filter);
    $filter = preg_replace("/nipu/", "**", $filter);
    $filter = preg_replace("/bohong/", "**", $filter);
    return $filter;
}
...
function getAvailableAdTheme(){
  $outt = "";
  foreach ($GLOBALS['availableAdTheme'] as $key => $adThemeId) {
